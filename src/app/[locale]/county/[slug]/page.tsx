...
const CountyPage = () => {
    ...
    const fetchCountyData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('County data fetched:', data.name, 'Slug:', countySlug)
          setCounty(data)
        } else {
          notFound()
        }
    ...
    const fetchCities = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('All cities fetched:', cities.length)
          console.log('Sample cities with county field:', cities.slice(0, 5).map((c: any) => ({ name: c.name, county: c.county })))
          
          // Check if cities have county field
          const citiesWithCounty = cities.filter((c: any) => c.county)
          const citiesWithoutCounty = cities.filter((c: any) => !c.county)
          console.log('Cities WITH county field:', citiesWithCounty.length)
          console.log('Cities WITHOUT county field:', citiesWithoutCounty.length)
          console.log('Sample cities without county:', citiesWithoutCounty.slice(0, 3).map((c: any) => c.name))
  ...
  const countyCities = useMemo(() => {
    ...
    const filtered = allCities.filter((city: any) => {
      ...
      console.log('City:', city.name, 'City County:', cityCounty, 'Target County:', targetCounty, 'Match:', cityCounty.includes(targetCounty))
      ...
      console.log('  County name match:', countyNameMatch, 'Slug match:', slugMatch, 'Final result:', countyNameMatch || slugMatch)
      
      return countyNameMatch || slugMatch
    })
    
    console.log('Total cities filtered for', county.name, ':', filtered.length)
        ...
        <section className="cities-section-beautiful" id="cities">
          <div className="cities-container-beautiful">
            <div className="section-header-beautiful">
              <span className="section-eyebrow-beautiful">Cities in {county.name}</span>
